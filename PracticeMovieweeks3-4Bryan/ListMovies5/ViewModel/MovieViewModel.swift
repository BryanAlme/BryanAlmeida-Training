...
protocol MovieViewModelType : AnyObject {
    var publisherMovies: Published<[Moviess]>.Publisher { get }
    ...
    func getMovie(by row: Int) -> Moviess?
...
class MovieModel : MovieViewModelType {
    var networkManager : NetworkManager
    private var subscribers = Set<AnyCancellable>()
    
    var publisherMovies: Published<[Moviess]>.Publisher { $movies }
    var publisherImages: Published<[Int: Data]>.Publisher { $images }
    var publisherFavourites: Published<[Int]>.Publisher { $favourites }
    
    
    
    @Published private(set) var movies  = [Moviess]()
    @Published private(set) var filteredMovies  = [Moviess]()
    ...
    func search(by searchText: String, onlyFavourites: Bool) -> Void {
        if(!searchText.isEmpty){
            filteredMovies = movies.filter({ (movie: Moviess) -> Bool in
    ...
    func getMovies(from url: String) {
        networkManager
            .getModel(ApiMovieResponses.self, from: url)
            .sink { completion in
